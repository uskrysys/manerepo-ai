...
with st.sidebar.form("quick_input_form", clear_on_submit=True):
    qi_type = st.radio("区分", ["支出", "収入"], horizontal=True, key="qi_type")
    qi_date = st.date_input("📅 日付", value=datetime.date.today(), key="qi_date")
    qi_cat_options = EXPENSE_CATEGORIES if qi_type == "支出" else INCOME_CATEGORIES
    qi_cat = st.selectbox("📂 カテゴリー", qi_cat_options, key="qi_cat")
    qi_amt = st.number_input("💰 金額 (円)", min_value=1, step=100, value=1000, key="qi_amt")
    qi_memo = st.text_input("📝 内容（摘要）", key="qi_memo")
    qi_tag = st.selectbox("🏷 性質", CONSUMPTION_TAGS, key="qi_tag") if qi_type == "支出" else "収入"
    qi_submitted = st.form_submit_button("✅ 登録", use_container_width=True)
    if qi_submitted:
        if qi_amt <= 0:
            st.error("⚠️ 金額は1円以上を入力してください。")
        elif qi_date > datetime.date.today():
            st.error("⚠️ 未来の日付は登録できません。")
        else:
            memo = qi_memo if qi_memo else "（未入力）"
...
with tab4:
    ...
    st.markdown('<div class="section-header">💾 データ編集・エクスポート</div>', unsafe_allow_html=True)
    st.caption("テーブルを編集すると画面上のサマリーやグラフに即座に反映されます。「💾 DB/CSVに永続保存」で確定保存します。")
